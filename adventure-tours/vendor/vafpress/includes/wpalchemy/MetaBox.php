...
class WPAlchemy_MetaBox
{
				...
				if ( $post_id == $post['post_id'] )
				{
					foreach( $post['postmeta'] as $meta )
					{
						if ( $key == $meta['key'] )
						{
							...
							//Fix for '/e' modificator usage
							$value = maybe_unserialize( preg_replace_callback( '!s:(\d+):"(.*?)";!s', array( $this, '_serialize_match_replacer' ), stripslashes( $meta['value'] ) ) );
	...
	function _serialize_match_replacer( $matches )
	{
