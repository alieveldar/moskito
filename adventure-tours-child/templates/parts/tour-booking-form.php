...
//unset($field_config['attribute_pa_extension']);
//unset($field_config['quantity_adult']);
...
TdJsClientScript::addScript( 'initTourBookingForm', 'Theme.tourBookingForm.init(' . wp_json_encode( array(
	...
	'dateCalendarAvailableTicketsMessage' => '',
...
<?php 
	...
	if ( $form_title ) {
		printf( '<h3 class="form-block__title">%s</h3><h4>Bitte wählen Sie Ihr Startdatum für diese Reise.</h4>', esc_html( $form_title ) );
	}
?>
	<form id="tourBookingForm" method="POST">
        ...
        <div class="form-block__buttons">
            <?php printf('<input class="booking" type="submit" value="%s">',
                esc_attr( apply_filters( 'adventure_tours_booking_form_btn_text', esc_html__( 'Book now', 'adventure-tours'), $product ) )
            ); ?>
            <?php printf('<input class="booking spu-open-'. (($post = get_post_by_slug('ask', 'spucpt')) ? $post->ID : 'NOT-FOUND') .'" type="button" value="%s">',
                esc_attr( esc_html__( 'Ask', 'adventure-tours-child') )
            ); ?>
        </div>
	</form>
</div>
